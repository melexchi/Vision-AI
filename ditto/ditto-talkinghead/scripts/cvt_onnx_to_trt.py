...
def _get_venv_trt_ld_path():
    """Get LD_LIBRARY_PATH pointing to venv's TRT libs to avoid system TRT conflict."""
    try:
        import tensorrt_libs
        return os.path.dirname(tensorrt_libs.__file__)
    except ImportError:
        return None
...
def onnx_to_trt(onnx_file, trt_file, fp16=False, more_cmd=None):
    ...
    if cap[0] >= 8:
        ...
        compatiable = ""
    cmd = [
        os.path.join(os.path.dirname(sys.executable), "polygraphy"),
        "convert",
        onnx_file,
        "-o",
        trt_file,
    ...
    if more_cmd:
        cmd = cmd + more_cmd
    # Prepend venv TRT libs to LD_LIBRARY_PATH so polygraphy uses the venv's
    # TRT version instead of any system-installed TRT.
    trt_libs = _get_venv_trt_ld_path()
    env_prefix = ""
    if trt_libs:
        existing = os.environ.get("LD_LIBRARY_PATH", "")
        env_prefix = f"LD_LIBRARY_PATH={trt_libs}:{existing} " if existing else f"LD_LIBRARY_PATH={trt_libs} "
    full_cmd = env_prefix + " ".join(cmd)
    print(full_cmd)
    os.system(full_cmd)
        ...
        if "GridSample" in layer.name:
            ...
            with warnings.catch_warnings():
                ...
                for j in range(layer.num_outputs):
                    layer.get_output(j).dtype = trt.float32
...
def main(onnx_dir, trt_dir, grid_sample_plugin_file="", force_fp16=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--force-fp16", action="store_true",
                        help="Build ALL models as FP16 (including lmdm, hubert, motion_extractor)")
    ...
    main(onnx_dir, trt_dir, grid_sample_plugin_file, force_fp16=args.force_fp16)
